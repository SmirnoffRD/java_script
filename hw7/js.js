...
const map = {
   ...
   */
  render(snakePointsArray, foodCell) {
...
const snake = {
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        nextPoint = {x: firstPoint.x, y: firstPoint.y - 1};
        if (this.isOuterPoint(nextPoint)) {
          return nextPoint;
        }
        return {x: firstPoint.x, y: game.config.getRowsCount() - 1};
      case 'right':
      ...
      case 'left':
        ...
        return {x: game.config.getColsCount() - 1, y: firstPoint.y};
    }
  },
  ...
  /**
   * Проверка на шаг
   * @param {{x: int, y: int}} nextStepPoint следующая клетка
   * @returns {boolean}
   */
  isOuterPoint(nextStepPoint) {
    return nextStepPoint.x < game.config.getColsCount() &&
    nextStepPoint.y < game.config.getRowsCount() &&
    nextStepPoint.x >= 0 &&
    nextStepPoint.y >= 0;
  },
...
const game = {
   ...
   */
  render() {
    this.map.render(this.snake.getBody(), this.map.cells[`x${this.food.x}_y${this.food.y}`]);
  },
  ...
  /**
   * Обновляет значение score на игровом поле
   */
  scoreSetter() {
    document.getElementById('game_score').textContent = `Очки: ${this.score}`;
  },
};
